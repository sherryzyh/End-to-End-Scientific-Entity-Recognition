...
if __name__ == '__main__':
    ...
    min_eval_loss = [10000, None]
    max_f1_score = [0, None]
    for e in range(epoch):
        eval_log = log_history[e * 2 + 1]
        if eval_log["eval_loss"] < min_eval_loss[0]:
            min_eval_loss = [eval_log["eval_loss"], eval_log["step"]]
        
        if eval_log["eval_f1"] > max_f1_score[0]:
            max_f1_score = [eval_log["eval_f1"], eval_log["step"]]
    ...
    print(f"Best f1 score: {max_f1_score[0]}, at step {max_f1_score[1]}")
    print(f"Min eval loss: {min_eval_loss[0]}, at step {min_eval_loss[1]}")